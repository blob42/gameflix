    ...
    evaluateScripts(javascriptFiles, function(success) {
        if (success) {
            ...
            youtubeDl.getSourceVideo(function(sourceVideo){
                ...
                //videoUrl = JSON.parse(JSON.stringify( sourceVideo ));
                //var newVideoUrl = [];
                //for (var i=0; i < videoUrl.length; i++) {
                    //newVideoUrl.push(videoUrl[i]);
                //}
                //console.log(md5(videoUrl));
                //console.log(this);
                //for (var i=0; i < sourceVideo; i++) {
                    //this.App.LOL[i] = sourceVideo[i];
                //}
                var source = JSON.parse(JSON.stringify( sourceVideo ));
                var mediaItem = new MediaItem("video", source);
                evaluateScripts([`${options.CLIENTURL}js/${sourceVideo}`], function(success){
                    URL();
                })
